...
public class AppointmentDB {
    ...
    public static ObservableList<Appointment> getMonthlyAppointments (int id) {
        ...
        try {
            ...
            String query = "SELECT * FROM appointment WHERE customerId = '" + id + "' AND " + "start >= '" + begin + "' AND start <= '" + end + "'"; 
            ResultSet results = statement.executeQuery(query);
            while(results.next()) {
                appointment = new Appointment(results.getInt("appointmentId"), results.getInt("customerId"), results.getString("start"),
                    results.getString("end"), results.getString("type"), results.getString("location"), results.getString("contact"));
    ...
    public static Appointment appointmentIn15Min() {
        ...
        try {
            ...
            if(results.next()) {
                appointment = new Appointment(results.getInt("appointmentId"), results.getInt("customerId"), results.getString("start"),
                    results.getString("end"), results.getString("type"), results.getString("location"), 
    ...
    public static boolean saveAppointment(int id, String type, String contact, String location, String date, String time) {
        ...
        try {
            Statement statement = Database.getConnection().createStatement();
            String query = "INSERT INTO appointment SET customerId='" + id + "', userId='1', title='not needed', description='not needed', contact='" 
                    + contact + "', type='" + type + "', location='" + location + "', start='" + tsStart + "', end='" + tsEnd + "', url='', "
    ...
    public static boolean updateAppointment(int id, String type, String contact, String location, String date, String time) {
        ...
        try {
            Statement statement = Database.getConnection().createStatement();
            String query = "UPDATE appointment SET customerId='" + id + "', userId='1', title='not needed', description='not needed', contact='" 
                    + contact + "', type='" + type + "', location='" + location + "', start='" + tsStart + "', end='" + tsEnd + "', url='', "
                    + "createDate=NOW(), createdBy=' ', lastUpdate=NOW(), lastUpdateBy=' '";
            int update = statement.executeUpdate(query);
            if(update == 1) {
    ...
    public static String createTimeStamp(String date, String time, String location, boolean startMode) {
        ...
        if(location.equals("New York")) {
            ...
            zid = ZoneId.of("America/Toronto");
        } else if(location.equals("Vancouver")) {
            zid = ZoneId.of("America/Vancouver");
        } else {
            zid = ZoneId.of("Europe/Oslo");
        }
